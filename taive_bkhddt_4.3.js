(function() {
    ...
    // Chuyển thành async function để dùng await
    async function downloadExcelFile(company, year, month, group) {
        let reports = [
        ...
        const filteredReports = reports.filter(r => (group === "all" || group === undefined || r.group === group));

        // Dùng vòng lặp for...of để đợi từng file
        for (const report of filteredReports) {
            ...
            try {
                ...
                // Nghỉ một chút (ví dụ 300ms) để trình duyệt xử lý lưu file trước khi tải tiếp
    ...
    async function downloadMonthRange(year, fromMonth, toMonth, company, group) {
		// Không cần document.getElementById ở đây nữa vì đã truyền qua tham số
		for (let i = fromMonth; i <= toMonth; i++) {
			await downloadExcelFile(company, year, i, group);
		}
		alert('Đã hoàn thành quá trình tải file!');
	}

    // --- PHẦN 2: GIAO DIỆN (Giữ nguyên logic UI của bạn) ---
    // (Lưu ý: các hàm onclick trong phần UI cần gọi downloadMonthRange mà không làm treo UI, 
    // nhưng vì đây là script chạy console/bookmarklet nên async trực tiếp là ổn)
    
    // ... [Đoạn mã UI giữ nguyên như file gốc của bạn] ...
    // --- CHỈ CẦN THAY ĐỔI CÁC LỜI GỌI HÀM TRONG UI THÀNH ASYNC ---
    ...
    function showDialog() {
		...
		for (let i = 1; i <= 12; i++) {
			...
			b.onclick = async () => { 
				...
				overlay.remove(); // Đóng UI
				
				// Gọi hàm tải với tham số đã lấy được
				await downloadMonthRange(y, i, i, cty, loai); 
			};
			mGrid.appendChild(b);
		}

		// 2. Đối với các nút Quý (Q1 - Q4)
		...
		for (let i = 1; i <= 4; i++) {
			...
			b.onclick = async () => { 
				...
				overlay.remove(); 
				await downloadMonthRange(y, (i-1)*3+1, i*3, cty, loai); 
			};
			qGrid.appendChild(b);
		}

		// 3. Đối với nút "Tải xuống" (Tùy chọn từ - đến)
		dialog.querySelector('#dl-fb').onclick = async () => {
			...
			overlay.remove();
			await downloadMonthRange(y, f, t, cty, loai);
		};
    }
