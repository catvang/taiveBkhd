...
function showDialog() {
	...
	if (!document.getElementById('inputMonthStyle')) {
	  const style = document.createElement('style');
	  style.id = 'inputMonthStyle';
	  style.innerHTML = `
		.input-month {
		  width: 60px;
		  padding: 8px 12px;
		...
		.input-month:focus {
		  box-shadow: 0 4px 16px #3182ce55;
		  border-color: #2b6cb0;
		}
	  `;
...
// Tạo hộp thoại đẹp hơn
dialog.style.background = '#f9fafb';
dialog.style.padding = '32px 24px';
dialog.style.borderRadius = '12px';
dialog.style.boxShadow = '0 4px 24px #00000030';
dialog.style.minWidth = '340px';
dialog.style.fontFamily = 'Segoe UI, Arial, sans-serif';

// Thêm tiêu đề
dialog.innerHTML = `
  <h2 style="margin-top:0;margin-bottom:16px;color:#2d3748;">Tải bảng kê hóa đơn</h2>
  <label>Mã Cty (dùng cho tên file tải về):</label><br>
...
<label for="loaiHd">Loại hóa đơn:</label><br>
<div style="display:flex;gap:18px;margin-bottom:12px;">
  ...
  <div style="display:flex;gap:12px;">
    <div>
      ...
      <input type="number" id="fromMonth" min="1" max="12" class="input-month">
    </div>
    <div>
      <label>Đến tháng:</label>
      <input type="number" id="toMonth" min="1" max="12" class="input-month">
      <button id="cungThangBtn" style="margin-left:8px;">(cùng tháng)</button>
    </div>
  </div>
  <br>
  
<label for="nam_bcao">Năm báo cáo:</label><br>
<div style="display:flex;gap:18px;margin-bottom:12px;">
  <label><input type="radio" name="nam_bcao" value="nam_0" checked> Năm nay</label>
  <label><input type="radio" name="nam_bcao" value="nam_1"> Năm trước</label>
  <label><input type="radio" name="nam_bcao" value="nam_2"> Hai Năm trước</label>
  <label><input type="radio" name="nam_bcao" value="nam_3"> Ba Năm trước</label>
</div>

  <strong>Chọn kỳ:</strong><br>
  ...
  <br>
  <button id="okBtn" style="background:#3182ce;color:#fff;padding:8px 16px;border:none;border-radius:6px;">OK</button>
  <button id="cancelBtn" style="margin-left:8px; color: #e53e3e;">Cancel</button>
`;
