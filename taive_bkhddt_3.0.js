...
function downloadPrevYearMonth(month) {
    const y = new Date().getFullYear() - 1;
    downloadMonthRange(y, month, month);
    document.getElementById('cancelBtn').click();
}

function downloadPrevYearQuarter(q) {
    const y = new Date().getFullYear() - 1;
...
function showDialog() {
    ...
    <br><br>
    <strong>Chọn nhanh năm trước:</strong><br>
  ...
  `;
    ...
    for (let m = 1; m <= 12; m++) {
        ...
        btn.textContent = `T${m}`;
        btn.onclick = () => downloadPrevYearMonth(m);
    ...
    for (let q = 1; q <= 4; q++) {
        ...
        btn.onclick = () => downloadPrevYearQuarter(q);
